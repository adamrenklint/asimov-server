...
module.exports = Base.extend({
  ...
  'logRunning': function (data) {
    ...
    var message = 'Running ' + time.trim() + ', received ' + numberWithCommas(self.totalRequestCount) + ' requests, throughput ' + numberWithCommas(total) + ' requests per ';
    message += reqPerSecond ? 'second' : 'minute';
  ...
  'start': function () {
    ...
    function initWorkers () {
      ...
      self.logger.pending(self.namespace, 'Starting server cluster with ' + count + ' workers @ http://127.0.0.1:' + port);
    ...
    tcpPortUsed.check(port).then(function (portInUse) {
      ...
      if (portInUse) {
        ...
        self.logger.pending(self.namespace, 'Port ' + port + ' is not available, standing by');
  ...
  'killCluster': function () {
    ...
    // TODO: this should be sending SIGHUP for disconnect/close?
    cluster.destroy();
  },
